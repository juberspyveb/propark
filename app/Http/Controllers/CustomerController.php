<?php
    ...
    use File, URL;
    use App\User;
    use App\Customer, App\NumberPlate;
    use Datatables;
    use Session;
    use PDF;
    ...
    class CustomerController extends Controller{
            ...
            return Datatables::of($customers)
                ...
                ->editColumn("action", function($customers) use($id) {
                    ...
                    if(!empty($id)){
                        ...
                        // $detail = '';
                        $detail = '<a href="'.route('admin-customer-detail',['id' => base64_encode($customers->plate_id)]).'"><i class="fas fa-eye text-success"></i></a>';
                    }
                    if($customers->status == config('constants.user_status.ACTIVE')) {
                        $status = '<a class="dropdown-item badge-gradient-warning" style="background-color: blue;" href="javascript:void( 0);" onclick="change_status(this);" data-status="2" data-id="'.$customers->plate_id.'">'.config('constants.user_status.INACTIVE').'</a>';
                    }else{
                        $status = '<a class="dropdown-item badge-gradient-success" href="javascript:void( 0);" onclick="change_status(this);" data-status="1" data-id="'.$customers->plate_id.'">'.config('constants.user_status.ACTIVE').'</a>';
                    }
                    
                    $html =
                            '<a href="'.route('admin-edit-customer',['id' => base64_encode($customers->plate_id)]).'"><i class="fas fa-pencil-alt text-success"></i></a>'.
                            $detail .
        ...
        public function insert(Request $request)
        {
            ...
            $last_inserted_id = $this->objCustomer->insertData($crud_data);

            if ($last_inserted_id > 0) {
                return redirect()->route('admin-customers-list')->with('success', __('Record inserted successfully.'));
            } else {
                return redirect()->route('admin-customer-add')->with('error', __('Failed to insert record.'))->withInput();
            }
        }
        /** insert */
        ...
        {
            $id = base64_decode($id);
            $exist = $this->objNumberPlate->getDatabyId($id);

            $this->validate(request(), [
                    'customer_mobile' => 'required|unique:customers,mobile,'.$exist->customer_id,
                    'number_plate' => 'required|unique:number_plats,number_plate,'.$exist->id,
                ]);
            ...
            $update =$this->objCustomer->updateData($crud_data, $exist->customer_id);
            $update =$this->objNumberPlate->updateData($np_crud_data, $id);
